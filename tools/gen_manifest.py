...
W('=================================')
W('ONLY THE THREE bgm_* FILES IN SECTION 1 ARE NEEDED.')
W('')
W('Everything else on this list is spoken by the device itself. Pashmak reads')
W('his lines aloud with the Android text-to-speech engine in a voice pitched')
W('up and slowed a little so it suits a small woolly creature, so the app')
W('talks with no recordings at all.')
W('')
W('The one catch is Persian: most phones ship Google TTS, which has no Persian')
W('voice, and then Pashmak goes quiet while the words still appear in his')
W('bubble. Settings says so in plain words and names the free engines that do')
W('speak Persian, so a parent can fix it in a minute. Nothing in the app is')
W('blocked either way.')
W('')
W('Every name below is an OVERRIDE. Drop a file in and that one line is played')
W('from the recording instead of being spoken — one line, a whole screen or the')
W('entire app, in any mix. This is how a real narrator can be added later')
W('without touching the code.')
W('')
W('Put the files in app/src/main/res/raw/ using exactly these names. Use .mp3')
W('or .ogg, mono, 44.1 kHz. File names must be lowercase with letters, digits')
W('and underscores only — aapt rejects anything else. Do not write the')
W('extension; the app looks the file up by name, so welcome.mp3 and')
W('welcome.ogg both work.')
W('')
